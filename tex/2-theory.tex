...
\begin{lemma}
    An epistemic calculus $\V$ cannot be both streongly epistemically conservative and fallible.
\end{lemma}
\begin{proof}
...
\begin{proof}
    ...
    which implies $x\leq y$ by cancellativity. Hence, $\{z\in \V \mid x\otimes z \leq y\}$ is empty, giving $[x,y] = \bot$. But, this implies that the defining formula $z\otimes x \leq y \iff z\leq [x,y] = \bot$ is only valid for $z = \bot$, which contradicts the non-triviality of $\V$. 
\end{proof}
...
\begin{proof}
    ...
    \end{align*}
    which is precicely the posterior odds. 
\end{proof}
